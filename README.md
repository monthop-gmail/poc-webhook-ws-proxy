...
Claude ใน Chat UI **ไม่ตอบแบบ streaming** (ข้อความโผล่ทีเดียวทั้งก้อน ไม่ไหลทีละตัวอักษร)

### สาเหตุ

MCP tool model ทำงานแบบ request-response:

```
Claude generate ข้อความทั้งหมด
    → call reply tool ครั้งเดียว
        → POST /webhook ครั้งเดียว
            → WebSocket ส่งทีเดียว → Chat UI แสดงทีเดียว
```

Claude Code ตัดสินใจเองว่าจะ call tool เมื่อไหร่ ไม่มีทาง intercept token stream ระหว่างการ generate ได้

### Workaround

ใส่ instruction ใน system prompt ให้ Claude ตอบทีละย่อหน้าโดย call `reply` หลายครั้ง — ไม่ใช่ true streaming แต่ทำให้ดูลื่นขึ้น:

```
ตอบทีละย่อหน้า โดย call reply ทีละครั้งก่อนไปย่อหน้าถัดไป
```

### True Streaming

ถ้าต้องการ streaming จริงต้องเปลี่ยน architecture โดยใช้ Claude API โดยตรง (ไม่ผ่าน Claude Code session) แล้ว POST แต่ละ chunk ไปที่ `/webhook` เอง
